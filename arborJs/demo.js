(function($){
  ...
  var Nav = function(elt){
    ...
    var sys = arbor.ParticleSystem(1000, 400, 0.9) // create the system with sensible repulsion/stiffness/friction
    sys.parameters({gravity:true}) // use center-gravity to make the graph settle nicely (ymmv)
    

	//The particle system tells arbor how much you want things to move around. 
	//(If you set the 3rd parameter to 1, nodes will not move around at all unless you drag and drop them). 
	//You can play around with values between 0 and 1 to see how flexible or rigid the graph becomes.
    ...
    sys.renderer = Renderer("#viewport") // our newly created renderer will have its .init() method called shortly by sys...
 	...
 	//This initializes the renderer which is used by arbor.js to draw the nodes on screen. 

	// var rootNode = sys.addNode('Animals',{'color':'red','shape':'dot','label':'Animals'});


	// var dog = sys.addNode('dog',{'color':'green','shape':'dot','label':'dog'});
	// var cat = sys.addNode('cat',{'color':'blue','shape':'dot','label':'cat'});

	// To indicate that the dog and cat are animals we connect them using edges.
	//If you wish to customize how the line is drawn you can edit the renderer to draw custom code. 

	// sys.addEdge(rootNode, dog,{length:.2});
	// sys.addEdge(rootNode, cat,{length:.2});
